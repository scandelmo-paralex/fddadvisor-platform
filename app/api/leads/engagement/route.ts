...
export async function GET(request: NextRequest) {
  try {
    ...
    // fdd_engagements.buyer_id stores auth.users.id (user.id from auth)
    // We need to use buyer_profiles.user_id to query engagements
    ...
    console.log('[DEBUG v4] engagementBuyerId:', engagementBuyerId, 'from buyer_profiles.user_id:', buyerProfile?.user_id, 'fallback accessRecord.buyer_id:', accessRecord.buyer_id)
