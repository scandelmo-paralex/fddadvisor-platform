-- Insert 4 test franchises with proper escaping
...
(
  ...
  NULL, NULL,
  98, 0,
  '["Second-largest hamburger chain globally with strong brand recognition", "Flame-grilling cooking method differentiates from competitors", "Comprehensive training and ongoing support programs"]'::jsonb
),
-- 7-Eleven
(
  '7-Eleven',
  ...
  NULL, NULL,
  45, 0,
  '["Largest convenience store chain with strong brand recognition", "24/7 operating model provides consistent revenue stream", "Comprehensive support and training programs"]'::jsonb
),
-- McDonalds (escaped apostrophe)
(
  E'McDonald\'s',
  ...
  NULL, NULL,
  67, 0,
  '["World''s largest restaurant chain with unmatched brand recognition", "Proven business model with decades of success", "Comprehensive training and support infrastructure"]'::jsonb
),
-- KFC Non-Traditional
(
  'KFC Non-Traditional',
  ...
  250, 50,
  12, 0,
  '["Globally recognized KFC brand with strong customer loyalty", "Captive audience in non-traditional venues", "Transparent financial performance disclosure (Item 19)"]'::jsonb
);

-- Verify the insert
SELECT name, franchise_score, total_units, initial_investment_low, initial_investment_high 
FROM franchises 
